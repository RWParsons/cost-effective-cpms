...
saveRDS(calibration_experiment_list, "output/calibration_simulations.rds")
# calibration_experiment_list <- readRDS("output/calibration_simulations.rds")
...
calibration_cowplot <- cowplot::plot_grid(
  plotlist = c(list(NULL), calibration_plots), 
  ncol=1, 
  rel_heights = c(0.075,rep(0.3, length(calibration_plots)))
)

# calibration_cowplot <- cowplot::plot_grid(plotlist=calibration_plots, ncol=1)
combined_fig <- cowplot::plot_grid(inb_cowplot, calibration_cowplot, ncol=2)


ggsave(filename="output/calibration_experiment/calibration_plots3.jpeg", plot=combined_fig, dpi=600, height=10.5, width=14)
...
p2 <- p +
  theme(
    plot.margin = margin(5.5, 5.5, 0, 0),
    axis.title.x = element_blank(),
    axis.text.x = element_blank(),
    axis.ticks.x = element_blank(),
...
plot_panels <- get_plot_component(p, "panel", return_all=T)
plot_strip <- get_plot_component(p, "strip", return_all=T)

# ggplot() + draw_grob(plot_strip[[1]]) + draw_grob(plot_strip[[2]], x=2, y=2)


gg_strips <- 
  ggplot() + 
  (gridExtra::arrangeGrob(grobs = plot_strip, nrow=1, padding=50) %>% draw_grob())
  
gg_panels <- 
  ggplot() + 
  (gridExtra::arrangeGrob(grobs = plot_panels, nrow=1, padding=50) %>% draw_grob())

insert_xaxis_grob(gg_panels, axis_canvas(plot=p)) %>% ggdraw()

axis_canvas(plot=p)

plot_grid(gg_strips, gg_panels, ncol=1, rel_heights = c(0.3,1)) + draw_grob(get_y_axis(p))

ggplot () + (gridExtra::arrangeGrob(plot_strip[[1]], plot_strip[[2]]) %>% draw_grob())
plot_grid(plotlist = draw_grob(plot_strip))


ggplot() + (layer_grob(p, 1)[[1]] %>% draw_grob()) + draw_grob(get_x_axis(p, position="bottom"))

ggplot() + (layer_grob(p, 1)[[1]] %>% draw_grob()) + draw_label("xlab")
...
saveRDS(falls_simulation, "output/im22-figures/falls_sim_data.rds")

icu_simulation <- 
  do_simulation(
    n_sims=5000, n_valid=10000, sim_auc=0.8, event_rate=0.01,
    fx_costs_training=get_nmb_est_ICU, fx_costs_evaluation=get_nmb_ICU
  )

# save primary analyses simulations
saveRDS(falls_simulation, "output/falls_primary_analyses.rds")
saveRDS(icu_simulation, "output/icu_primary_analyses.rds")

# load primary analyses
falls_simulation <- readRDS(falls_simulation, "output/falls_primary_analyses.rds")
icu_simulation <- readRDS(icu_simulation, "output/icu_primary_analyses.rds")
...
p_falls_nmb <-
  ...
  rename(any_of(cols_rename)) %>%
  plot_fw_histogram(hdi=F, plot_labels=labs(x="Incremental Net Monetary Benefit (AUD)\n", y=""), inb_ref_col='Treat All')
...
primary_analyses_table <- 
  ...
  pack_rows(index=c("ICU readmission"=nrow(icu_summary), "Inpatient falls"=nrow(falls_summary))) %>%
  kable_styling()
...
saveRDS(ll_icu, "output/icu_sensitivity_analyses.rds")

# load simulation results
# ll_falls <- readRDS("output/falls_sensitivity_analyses.rds")
# ll_icu <- readRDS("output/icu_sensitivity_analyses.rds")


# save INB plots
cowplot::plot_grid(
  plotlist=get_plot_list(ll_falls, cols_rename, get_what="inb", inb_ref_col="Treat All"), 
  ncol=length(unique(g_falls$sim_auc))
)
ggsave(filename="output/sensitivity_analyses/falls_simulations_inb2.jpeg", height=24*0.5, width=12)

cowplot::plot_grid(
  plotlist=get_plot_list(ll_icu, cols_rename, get_what="inb", inb_ref_col="Treat None"), 
  ncol=length(unique(g_icu$sim_auc))
)
ggsave(filename="output/icu_simulations_inb.jpeg", height=24, width=12)
...
make_table(
  ...
  save_path="output/falls_inb_summary.html", inb_ref_col="Treat All"
)
make_table(
  ll_falls, get_what="cutpoints", rename_vector=cols_rename, 
  save_path="output/falls_thresholds_summary.html"
)

make_table(
  ll_icu, get_what="inb", rename_vector=cols_rename, 
  save_path="output/icu_inb_summary.html", inb_ref_col="Treat None"
)
make_table(
  ll_icu, get_what="cutpoints", rename_vector=cols_rename, 
  save_path="output/icu_thresholds_summary.html"
)
...
ggsave(filename="output/im22-figures/falls-sensitivity-analyses_fall-rate_nmb.jpeg", height=6.5, width=7)
...
# fall rate
ll_event_rate <- readRDS("output/im22-figures/falls-sensitivity-analyses_fall-rate.rds")
event_rate_plot <- cowplot::plot_grid(
  plotlist=get_plot_list(ll_event_rate, cols_rename, get_what="inb", inb_ref_col="Treat All", remove_axis=T, label_wrap_width=20) %>% keep_only_first_plot_strip(), 
  ncol=1,
  labels=scales::percent(g_event_rate$event_rate), 
  hjust=0
)
saveRDS(event_rate_plot, "output/im22-figures/event_rate_plot.rds")
ggsave(filename="output/im22-figures/falls-sensitivity-analyses_fall-rate_nmb.jpeg", height=10.5, width=7)

# AUC
ll_auc <- readRDS("output/im22-figures/falls-sensitivity-analyses_AUC.rds")
auc_plot <- cowplot::plot_grid(
  plotlist=get_plot_list(ll_auc, cols_rename, get_what="inb", inb_ref_col="Treat All", remove_axis=T, label_wrap_width=20) %>% keep_only_first_plot_strip(), 
  ncol=1,
  labels=g_auc$sim_auc, 
  hjust=0
)
saveRDS(auc_plot, "output/im22-figures/auc_plot.rds")
ggsave(filename="output/im22-figures/falls-sensitivity-analyses_AUC_nmb.jpeg", height=10.5, width=7)

# p <- plot_fw_histogram(data=ll_auc[[1]]$df_result, agg_line_size=0.5) +
#   theme_minimal() + 
#   theme(
...
#     panel.background = element_rect(fill = "transparent", linetype=0)
#   )
# 
sticker(
  p,

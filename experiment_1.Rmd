---
title: "Experiment 1"
date: "`r format(Sys.time(), '%d %B, %Y')`"
output: rmarkdown::github_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE, warning = FALSE)
set.seed(42)
library(tidyverse)
library(data.table)
source("src/utils.R")
```


Question: What are the differences in NMB between models where the Probability threshold was based on the currently available methods versus costs-based selection. (Hospital falls as a use case.)

1. Define costs of a TP, TN, FP, FN of falls classification (option to move this into the loop where costs are sampled from a distributions to account for uncertainty in their estimates in the literature)
    * FP have cost of applying intervention
    * FN have cost of patient fall
    * TP have cost of intervention + cost of fall*(1-effectiveness of intervention on rate of falls)
    * TN are cost $0
2. Select appropriate AUC (~0.75?) and prevalence (~3% ) for comparable clinical prediction model for falls.
3. For sample sizes (N) in [100, 500, 1000]: (repeat 500 times at each sample size)
    * Get training data by sampling observed predictor values and outcome by transforming AUC into Cohens' D and sampling from two normal distributions, the first (negative events) with mean=0 and the second (positive events) with mean=Cohens'D. (Both with sd=1.)
    * Fit a logistic regression model using this sampled data.
    * Fit predicted probabilities to the training data and use these to obtain probability thresholds using each method.
    * Get validation data using the same approach but with n=1000.
    * Use the previously fit model to estimate probabilities for validation data.
    * Evaluate the thresholds selected using the training data on the validation data, in terms of mean cost per patient.
4. Measure differences in NMB on validation sample dependent on use of currently available methods and cost-based approach to determine threshold.
5. Observe whether this relationship is dependent on the sample size taken
6. ???

```{r, include=FALSE, results='hide'}
# space for looking at distributions to estimate what is reasonable to use in get_nmb()

p = seq(0,1, length=100)
plot(p, dbeta(p, 10, 40), type='l')

get_beta(alpha=10, p=0.2)

(10/3)/(30+(10/3))
```


### Define costs
```{r}

# get_costs <- function(){
#   treatment_effect <- rbeta(1, 111.111, 1000)
#   QALYs_event <- rbeta(1, 10/3, 30)
#   treatment_cost <- rgamma(1, 250)
#   wtp <- 28000
#   
#   c(
#     "TN"=0, 
#     "FN"=QALYs_event*wtp, 
#     "TP"=QALYs_event*(1-treatment_effect)*wtp + treatment_cost, 
#     "FP"=treatment_cost
#   )
# }
# get_costs()

get_nmb <- function(){
  treatment_effect <- rbeta(1, 111.111, 1000) #QALY increment from treatment
  QALYs_event <- -rbeta(1, 10/3, 30) #QALY decrement from event
  treatment_cost <- rgamma(1, 100)
  wtp <- 28000
  
  c(
    "TN"=0,
    "FN"=QALYs_event*wtp,
    "TP"=QALYs_event*(1-treatment_effect)*wtp - treatment_cost,
    "FP"=-treatment_cost
  )
}
get_nmb()

```


### Run simulation
```{r}

do_simulation <- function(sample_size, n_sims, n_valid, sim_auc, event_rate, 
                          fx_costs, resample_values=TRUE, 
                          get_what=c("data", "plot"), plot_type="boxplot",
                          seed=42){
  if(!is.null(seed)) set.seed(seed)
  df_result <- data.frame()
  df_thresholds <- data.frame()
  
  i <- 0
  while(i < n_sims){
    train_sample <- get_sample(auc=sim_auc, n_samples=sample_size, prevalence=event_rate)
    valid_sample <- get_sample(auc=sim_auc, n_samples=n_valid, prevalence=event_rate)
    if(length(unique(train_sample$actual))!=2 | length(unique(valid_sample$actual))!=2){
      next
    }
    i <- i + 1
    model <- glm(actual~predicted, data=train_sample, family=binomial())
    train_sample$predicted <- predict(model, type="response")

    
    valid_sample$predicted <- predict(model, type="response", newdata=valid_sample)
    
    value_vector <- fx_costs()
    
    thresholds <- get_thresholds(
      predicted=train_sample$predicted, 
      actual=train_sample$actual,
      costs=value_vector
    )
    
    if(resample_values){
      value_vector <- fx_costs()
    }
    
    cost_threshold <- function(pt){
      classify_samples(
        predicted=valid_sample$predicted,
        actual=valid_sample$actual,
        pt=pt,
        costs=value_vector
      )
    }
    
    df_result <- rbind(
      df_result, 
      data.frame(
        n_sim=i, 
        treat_all=cost_threshold(0),
        treat_none=cost_threshold(1),
        cost_effective_cost=cost_threshold(thresholds$pt_cost_effective),
        youden_cost=cost_threshold(thresholds$pt_youden), 
        cz_cost=cost_threshold(thresholds$pt_cz), 
        iu_cost=cost_threshold(thresholds$pt_iu),
        er_cost=cost_threshold(thresholds$pt_er)
      )
    )
    
    df_thresholds <- rbind(
      df_thresholds, 
      data.frame(
        n_sim=i, 
        treat_all=0,
        treat_none=1,
        cost_effective_pt=thresholds$pt_cost_effective,
        youden_pt=thresholds$pt_youden, 
        cz_pt=thresholds$pt_cz,
        iu_pt=thresholds$pt_iu,
        er_pt=thresholds$pt_er
      )
    )
  }
  return(df_result)
  pts <- round(colMeans(df_thresholds)[-1], 3)
  df_plot <- df_result
  names(df_plot)[-1] <- method_levels <- paste0(names(df_plot)[-1], "(", pts, ")")
  
  if(plot_type=="boxplot"){
    p <- 
      df_plot %>%
      pivot_longer(!n_sim, names_to="method", values_to="nmb") %>%
      mutate(method=str_remove(method, "_cost"),
             method=factor(method, levels=str_remove(method_levels, "_cost"))) %>%
      
      ggplot(aes(method, nmb)) +
      geom_boxplot() +
      geom_jitter(alpha=0.1, width=0.1) +
      theme_bw() +
      labs(
        # y="Mean cost per patient",
        # x="Probability threshold selection method",
        y="", x="",
        subtitle=glue::glue("auc: {sim_auc}; event_rate: {event_rate}")
      ) +
      scale_y_continuous(labels=scales::dollar_format()) +
      theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
  }else{
    get_medians <- function(x, nboot=500){
      m <- median(x)
      
      boots <- map_dbl(
        1:nboot, 
        function(v){
          s <- sample(x, size=length(v), replace=T)
          median(s)
        }
      )
      list(median=m, median_se=sd(boots))
    }
    
    df_plot2 <- 
      df_plot %>%
      pivot_longer(!n_sim, values_to="nmb", names_to="method")
    
    
    df_plot2 <- as.data.table(df_plot2)[
      ,
      get_medians(x=nmb),
      by = list(method)
    ]
    
    p <-
      df_plot2 %>%
      mutate(method=str_remove(method, "_cost"),
             method=factor(method, levels=str_remove(method_levels, "_cost"))) %>%
      ggplot(aes(method, median)) +
      geom_point() +
      geom_errorbar(aes(ymin=median-median_se, ymax=median+median_se, width=0)) +
      theme_bw() +
      labs(
        # y="Mean cost per patient",
        # x="Probability threshold selection method",
        y="", x="",
        subtitle=glue::glue("auc: {sim_auc}; event_rate: {event_rate}")
      ) +
      scale_y_continuous(labels=scales::dollar_format()) +
      theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
  }
  
  if(length(get_what)==1){
    if(get_what=="data"){return(df_result)}
    if(get_what=="plot"){return(p)}
  }else{
    return(list(data=df_result, plot=p))
  }
}

do_simulation(
  sample_size=500, n_sims=50, n_valid=500, sim_auc=0.65, event_rate=0.03,
  fx_costs=get_nmb, resample_values=TRUE, get_what=c("data", "plot"), plot_type = "point"
)

# p_list <- lapply(c(0.03, 0.1, 0.5, 0.9, 0.97), function(x)do_simulation(sample_size=500, n_sims=100, n_valid=1000, sim_auc=0.65, event_rate=x, fx_costs=get_costs, get_what="plot"))
# p_list
```

## search through a grid of combinations of AUC and event rates to see how this influences the differences between probability threshold methods. The same costs were used in all simulations (distributions at top of document) and are resampled separately for training and validation.
#### In the plot below, the columns, from left to right, have increasing AUC. The rows, from top to bottom, have increasing event rates.

### My hot takes:
#### Probability threshold selection method becomes increasingly important as the AUC of the model and the event rate reduces.
#### I think that this is because, for models with very high discrimination, they're able to correctly classify a larger proportion of samples, and there are fewer which are classified differently based on selection method. The difference is greater for smaller event rates because a false negative is the most costly classification, and only the cost-based method is "aware" of this. This is also why, when the event rate is very high, there is not much of a difference between methods (cost-based method is focused on correctly classifying the majority class but so are the other methods).
```{r, fig.height=24, fig.width=12}

library(parallel)
n_cluster <- detectCores()
cl <- makeCluster(n_cluster)
cl <- parallelly::autoStopCluster(cl)

g <- expand.grid(
  sim_auc=c(0.65, 0.75, 0.85, 0.95),
  event_rate=c(0.01, 0.03, 0.1, 0.3, 0.7, 0.9)
)

clusterExport(cl, {
  c("do_simulation", "g", "get_nmb")
})

invisible(clusterEvalQ(cl, {
  library(tidyverse)
  library(data.table)
  source("src/utils.R")
}))


ll1 <- parallel::parLapply(
  cl,
  1:nrow(g),
  function(i) do_simulation(
    sample_size=500, n_sims=100, n_valid=1000,
    sim_auc=g$sim_auc[i], event_rate=g$event_rate[i],
    fx_costs=get_nmb, get_what="plot", resample_values=TRUE,
    plot_type="point"
  )
)

ll2 <- parallel::parLapply(
  cl,
  1:nrow(g),
  function(i) do_simulation(
    sample_size=500, n_sims=100, n_valid=1000,
    sim_auc=g$sim_auc[i], event_rate=g$event_rate[i],
    fx_costs=get_nmb, get_what="plot", resample_values=FALSE,
    plot_type="point"
  )
)

cowplot::plot_grid(plotlist=ll1, ncol=length(unique(g$sim_auc)))

cowplot::plot_grid(plotlist=ll2, ncol=length(unique(g$sim_auc)))

```

```{r, include=FALSE, fig.height=24, fig.width=12}
# out <- map2(
#   .x=g$sim_auc, .y=g$event_rate, 
#   function(.x, .y) 
#     do_simulation(
#       sample_size=500, n_sims=100, n_valid=1000, 
#       sim_auc=.x, event_rate=.y, fx_costs=get_costs, get_what="plot"
#     )
# )
# cowplot::plot_grid(plotlist=out, ncol=length(unique(g$sim_auc)))
```

# {cutpointr} offers a function `total_utility()` which is very similar to our proposed method. 
If we highjack it a bit as I do below, then it offers several optimisation methods to find the maximum for our given function.  I pass fx_total_nmb() and give it the same arguments as cutpointr::total_utility() so that it works the way as our existing approach does. They both find very similar cutpoints using the same data and nmb_vector.
```{r}
library(cutpointr)
fx_total_nmb <- function(tn, tp, fn, fp, utility_tp, utility_tn, cost_fp, cost_fn, ...){
  tn*utility_tn + tp*utility_tp + fn*cost_fn + fp*cost_fp
}

cutpointr(data=x, x=predicted, class=actual, method=maximize_metric, metric=fx_total_nmb, utility_tp=nmb_vec["TP"], utility_tn=nmb_vec["TN"], cost_fp=nmb_vec["FP"], cost_fn=nmb_vec["FN"])[['optimal_cutpoint']]

get_thresholds(x$predicted, x$actual, costs=nmb_vec)$pt_cost_effective

```


#### everything below here are (unsuccessful) attempts to optimise code so that passing new functions to get probability thresholds is easier. 

It does work, but it makes the code much slower, so probably not worth it...
```{r}
source("src/cutpoint_methods.R")
x <- get_beta_preds(alpha=1, p=0.1, n=1000)$preds 

x$new <- 1 - abs(x$predicted - 0.1)

nmb_vec <- get_nmb()


get_thresholds2 <- function(predicted, actual, nmb, metrics, pt_seq=seq(0.01, 0.99, 0.01)){
  rocobj <- pROC::roc(as.factor(actual), predicted, direction="<", quiet=TRUE)
  auroc <- pROC::auc(rocobj)
  cms <- map_dfr(pt_seq, function(p) get_confusion(predicted=predicted, actual=actual, pt=p))
  cms <- as.matrix.data.frame(cms)
  # return(cms)
  parms_list <- list(
    roc=rocobj, 
    auc=auroc,
    predicted=predicted, 
    actual=actual, 
    pt_seq=pt_seq, 
    confusion_matrices=cms, 
    nmb=nmb,
    f_select=median
  )
  
  # return(purrr::map(metrics, function(m) do.call(m, parms_list)))
  # return(do.call("get_nmb_pt", parms_list))
  # return(list(metrics=metrics, parms_list=parms_list))
  l <- unlist(purrr::map(metrics, function(m) do.call(m, parms_list)))
   sapply(metrics, function(m) do.call(m, parms_list))
}

obj <- get_thresholds2(predicted=x$predicted, actual=x$actual, nmb=nmb_vec, metrics=c("get_er_pt", "get_youden_pt", "get_iu_pt", "get_nmb_pt", "get_cz_pt"))



f1 <- function(){
  get_thresholds2(predicted=x$predicted, actual=x$actual, nmb=nmb_vec, metrics=c("get_er_pt", "get_youden_pt", "get_iu_pt", "get_nmb_pt", "get_cz_pt"))
}

f2 <- function(){
  get_thresholds(predicted=x$predicted, actual=x$actual,costs=nmb_vec)
}

profvis({
  # get_thresholds2(predicted=x$predicted, actual=x$actual, nmb=nmb_vec, metrics=c("get_er_pt", "get_youden_pt", "get_iu_pt", "get_nmb_pt", "get_cz_pt"))
  get_thresholds(predicted=x$predicted, actual=x$actual,costs=nmb_vec)
})




microbenchmark::microbenchmark(
  new=f1(),
  old=f2()
)



f1 <- function(){
  cms <- matrix(NA, nrow=length(seq(0,1,0.01)), ncol=6)
  for(i in 1:nrow(cms)){
    cms[i,] <- unlist(get_confusion(d=data.frame(predicted=x$predicted, actual=x$actual), pt=seq(0,1,0.01)[i]))
  }
  return(cms)
}

f2 <- function(){
  cms <- map_dfr(seq(0,1,0.01), function(p) get_confusion(d=data.frame(predicted=predicted, actual=actual), pt=p))
  as.matrix.data.frame(cms)
}

microbenchmark::microbenchmark(
  f1, f2
)

f1()
unlist(get_confusion(d=data.frame(predicted=x$predicted, actual=x$actual), pt=0.01))
vapply(seq(0,1,0.01), function(p) get_confusion(d=data.frame(predicted=x$predicted, actual=x$actual), pt=p))


```




```{r}

get_nmb()
cms <- get_thresholds2(
  predicted=x$predicted,
  actual=x$actual,
  nmb=get_nmb(), metrics=c("get_cz_pt")
)
#TODO
# check speed for calculating metrics with confusion matrices as one big matrix!
# update into new get_thresholds2

get_cz_pt <- function(predicted, actual, pt_seq, confusion_matrices, f_select, ...){
  out <- confusion_matrices[,"Se"]*confusion_matrices[,"Sp"]
  f_select(pt_seq[out==max(out)])
}

out <- get_cz_pt(predicted=x$predicted, actual=x$actual, pt_seq=seq(0,1,0.01), confusion_matrices = cms, f_select=median)

row <- get_thresholds2(
  predicted=x$predicted,
  actual=x$actual,
  nmb=get_nmb(), metrics=c("get_er_pt","get_youden_pt","get_cz_pt","get_iu_pt","get_nmb_pt")
)

profvis({
  get_thresholds2(
    predicted=x$predicted,
    actual=x$actual,
    nmb=get_nmb(), metrics=c("get_er_pt","get_youden_pt","get_cz_pt","get_iu_pt","get_nmb_pt")
  )
})

microbenchmark::microbenchmark(
  new=get_thresholds2(
    predicted=x$predicted,
    actual=x$actual,
    nmb=get_nmb(), metrics=c("get_er_pt","get_youden_pt","get_cz_pt","get_iu_pt","get_nmb_pt")
  ),
  old=get_thresholds(
    predicted=x$predicted,
    actual=x$actual,
    costs=get_nmb())
)


get_thresholds(
  predicted=x$predicted,
  actual=x$actual,
  costs=get_nmb())

# df <- data.frame(matrix(NA, ncol=8, nrow=500))

# df[1,4:8] <- row

```


```{r}

# source("src/cutpoint_methods.R")
# source("src/utils.R")

do_simulation2 <- function(
  sample_size, n_sims, n_valid, sim_auc, event_rate, 
  fx_costs, resample_values=TRUE, 
  get_what=c("data", "plot"), plot_type="boxplot",
  cutpoint_methods=c("get_er_pt","get_youden_pt","get_cz_pt","get_iu_pt","get_nmb_pt"),
  seed=42){
  
  if(!is.null(seed)) set.seed(seed)
  n_cols <- 3 + length(cutpoint_methods)
  df_result <- matrix(NA, ncol=n_cols, nrow=n_sims)
  df_thresholds <- matrix(NA, ncol=n_cols, nrow=n_sims)
  # return(df_thresholds)
  colnames(df_result) <- 
    cutpoint_methods %>% 
    str_remove(., "^get_") %>%
    str_replace(., "pt$", "nmb") %>% 
    c("n_sim", "treat_all_nmb", "treat_none_nmb", .)
  
  colnames(df_thresholds) <- 
    cutpoint_methods %>% 
    str_remove(., "^get_") %>%
    c("n_sim", "treat_all_pt", "treat_none_pt", .)
  
  df_result[,"n_sim"] <- df_thresholds[,"n_sim"] <- 1:n_sims
  # return(df_result)
  df_thresholds[,"treat_all_pt"] <- 0
  df_thresholds[,"treat_none_pt"] <- 1
  
  i <- 0
  while(i < n_sims){
    train_sample <- get_sample(auc=sim_auc, n_samples=sample_size, prevalence=event_rate)
    valid_sample <- get_sample(auc=sim_auc, n_samples=n_valid, prevalence=event_rate)
    if(length(unique(train_sample$actual))!=2 | length(unique(valid_sample$actual))!=2){
      next
    }
    i <- i + 1
    model <- glm(actual~predicted, data=train_sample, family=binomial())
    train_sample$predicted <- predict(model, type="response")
    valid_sample$predicted <- predict(model, type="response", newdata=valid_sample)
    
    value_vector <- fx_costs()
    
    # thresholds <- get_thresholds2(
    #   predicted=train_sample$predicted, 
    #   actual=train_sample$actual,
    #   nmb=value_vector,
    #   metrics=cutpoint_methods
    # )
    
    thresholds <- get_thresholds(
      predicted=train_sample$predicted, 
      actual=train_sample$actual,
      costs=value_vector
    )
    # return()
    df_thresholds[i, 4:n_cols] <- unlist(thresholds)
    # return(df_thresholds)
    if(resample_values){
      value_vector <- fx_costs()
    }
    
    get_valid_nmb <- function(pt){
      classify_samples(
        predicted=valid_sample$predicted,
        actual=valid_sample$actual,
        pt=pt,
        costs=value_vector
      )
    }
    # return(map_dbl(df_thresholds[i, 2:n_cols], get_valid_nmb))
    
    df_result[i, 2:n_cols] <- map_dbl(df_thresholds[i, 2:n_cols], get_valid_nmb)
    
  }
  return(df_result)
}


do_simulation2(sample_size=1000, n_sims=10, n_valid=500, sim_auc=0.8, event_rate=0.1, 
  fx_costs=get_nmb)
# mat[1, 2:8]

# t1 <- Sys.time()
# profvis({
#   do_simulation2(sample_size=1000, n_sims=100, n_valid=500, sim_auc=0.8, event_rate=0.1, 
#   fx_costs=get_nmb)
# })
# 
# 
profvis({
  do_simulation(sample_size=1000, n_sims=100, n_valid=500, sim_auc=0.8, event_rate=0.1,
  fx_costs=get_nmb)
})
t1 <- Sys.time()
do_simulation2(sample_size=1000, n_sims=50, n_valid=500, sim_auc=0.8, event_rate=0.1,
  fx_costs=get_nmb)
print(Sys.time()-t1)
t1 <- Sys.time()
do_simulation(sample_size=1000, n_sims=50, n_valid=500, sim_auc=0.8, event_rate=0.1,
  fx_costs=get_nmb)
print(Sys.time()-t1)
```


```{r}
classify_samples2 <- function(predicted, actual, pt, costs){
  # predicted: vector of predicted probabilities
  # actual: binary label to the event
  # pt: probability threshold used to classify predicted probabilities into classes
  # costs:  named vector containing costs for each possible correct or incorrect classification (2x2)
  #         for example: costs <- c("TN"=0, "FN"=100, "TP"=80, "FP"=5)

  d <- cbind(predicted, actual, NA)
  colnames(d) <- c("predicted", "actual", "nmb")
  
  d[d[,"predicted"] < pt & d[,"actual"]==0, "nmb"] <- costs["TN"]
  
  d[d[,"predicted"] < pt & d[,"actual"]==1, "nmb"] <- costs["FN"]
  d[d[,"predicted"] > pt & d[,"actual"]==1, "nmb"] <- costs["TP"]
  d[d[,"predicted"] > pt & d[,"actual"]==0, "nmb"] <- costs["FP"]

  mean(d[,"nmb"])
}
x <- get_beta_preds(alpha=1, p=0.1, n=1000)$preds


classify_samples2(predicted=x$predicted, actual=x$actual, pt=0.1, costs=get_nmb())

microbenchmark::microbenchmark(
  classify_samples2(predicted=x$predicted, actual=x$actual, pt=0.1, costs=get_nmb()),
  classify_samples(predicted=x$predicted, actual=x$actual, pt=0.1, costs=get_nmb())
)


d <- cbind(x$predicted, x$actual, NA)
colnames(d) <- c("predicted", "actual", "nmb")

d[d['predicted'] < 0.1, 'nmb'] <- 0
head(d)

d[1:3,c("nmb")] <- 3
d[,"nmb"] %>% head()


tn_idx <- d[,"predicted"] < 0.1
d[tn_idx, "nmb"] <- 1
d %>% head(20)



get_smooth_max <- function(x, y){
  smooth <- do.call(supsmu, data.frame(x, y))
  max.idx <- which.max(smooth$y)
  smooth$x[max.idx]
}


get_smooth_max2 <- function(x, y){
  smooth <- do.call(supsmu, list(x, y))
  max.idx <- which.max(smooth$y)
  smooth$x[max.idx]
}

microbenchmark::microbenchmark(
  get_smooth_max(x=rnorm(100), y=rnorm(100)),
  get_smooth_max2(x=rnorm(100), y=rnorm(100))
)
get_smooth_max(x=rnorm(100), y=rnorm(100))





```

...
```{r, include=FALSE}
costs <- c("TN"=0, "FN"=100, "TP"=80, "FP"=5)

df_preds <- get_beta_preds(alpha=get_alpha(beta=2.5, p=0.1), beta=2.5, p=0.1, n=10000, get_what="preds")$preds

df_pt_costs <- data.frame(pt=seq(0.05, 0.95,0.01))
df_pt_costs$mean_cost <- map_dbl(
  df_pt_costs$pt, 
  function(x) classify_samples(
    predicted=df_preds$predicted, 
    actual=df_preds$actual, pt=x, costs=costs
  )
)
df_pt_costs %>%
  ggplot(aes(pt, mean_cost)) + geom_line() +
  geom_point(data=slice(arrange(df_pt_costs, mean_cost),1), aes(pt, mean_cost)) +
  labs(subtitle=glue::glue(
    "lowest cost at pt of {slice(arrange(df_pt_costs, mean_cost),1)$pt}\n",
    "at a cost of ${slice(arrange(df_pt_costs, mean_cost),1)$mean_cost}"
    )
  )
```
